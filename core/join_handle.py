...
class JoinHandle:
    ...
    @staticmethod
    def _clean_ids(ids: list | None) -> list[str]:
        return [
            qq
            for item in ids or []
    ...
    async def _send_private(
        ...
        for user_id in user_ids:
            try:
                ...
                logger.error(f"无法发送消息给{target_name} {user_id}：{e}")
        return sent

    async def _send_admin(self, client: CQHttp, message: str):
        await self._send_private(client, self.cfg.admins_id, message, "bot管理员")
    ...
    async def event_monitoring(self, event: AiocqhttpMessageEvent):
        ...
        # 主动退群事件
            ...
            leave_notify = await self.db.get(gid, "leave_notify", False)
